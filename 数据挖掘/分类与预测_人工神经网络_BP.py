"""
    ...
    BP神经网络是一种处理分类和回归问题的有效的神经网络

    反向传播算法：
        1、 利用前向传播算法，得到Ln输出层的激活函数alpha
        2、 计算输出层（第n_i层）的残差
        3、 计算 l=n_i-1, ni-2,……,2 的各层残差
        4、 计算最终需要的偏导数值
        5、 使用梯度下降法更新权重参数
"""
"""
    我们尝试使用BP神经网络进行实验
    数据采用scikit-learn提供的make_moons数据集
    将数据分成训练集和测试集，使用训练集训练神经网络
    将训练好的神经网络作用于测试集，得到预测的错误率
...
print("=" * 100)
print(trainingSet[0:5, :])
print("-" * 100)
print(trainingLabels[0:5])
print("=" * 100)
