"""
    ...
    数据采用UCI数据库的疝气病症预测病马数据

    数据有多行，每行都有22个数据，前21个为马的病症数据，最后一个为该马的标签

    数据已被分为训练集和测试集 horseColicTraining.txt 和 horseColicTest.txt

    本实验任务使用训练集训练BP神经网络并预测测试集的标签

    并尝试将预测的测试集的标签错误率控制在30%以下
"""
...
# 获取数据并分为训练集与测试集
trainingData = pd.read_csv('horseColicTraining.txt', sep='\t')
testData = pd.read_csv('horseColicTest.txt', sep='\t')
trainingSet = trainingData.iloc[:, 0:-1].values
trainingLabels = trainingData.iloc[:, -1].values
testSet = testData.iloc[:, 0:-1].values
testLabels = testData.iloc[:, -1].values
print("=" * 100)
print(trainingSet[0:4, :])
print("-" * 100)
print(trainingLabels[0:4])
print("=" * 100)

# 绘出数据图
# plt.scatter(trainingSet[trainingLabels == 1][:, 0],
#             trainingSet[trainingLabels == 1][:, 1],
#             s=40, c='r', marker='x',
#             cmap=plt.cm.Spectral)
# plt.scatter(trainingSet[trainingLabels == 0][:, 0],
#             trainingSet[trainingLabels == 0][:, 1],
#             s=40, c='y', marker='+',
#             cmap=plt.cm.Spectral)
# plt.show()
...
# trainingSet, testSet, trainingLabels, testLabels = \
#     train_test_split(trainingSet,
#                      trainingLabels,
#                      test_size=.8,
#                      random_state=42)

# 设置网络参数
layer = [trainingData.shape[1]-1, 10, 1]  # 设置层数和节点数
Lambda = 0.005  # 正则化系数
alpha = 0.2  # 学习速率
...
print('训练样本的未正则化的函数值:', costfunction(predict, np.array(trainingLabels)))

print('训练样本错误率: %.2f%%' % ((error_rate(predict, np.array(trainingLabels)))*100))
...
print('测试样本错误率: %.2f%%' % (error_rate(predict, np.array(testLabels))*100))
